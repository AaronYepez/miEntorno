...
2. Crea y activa el entorno virtual:
   ...
   ```
3. Actualiza pip y instala las dependencias:
   ```powershell
...
5. Ajusta los valores en `.env`:
   ...
   - `SMTP_PASSWORD`
   - `EMAIL_FROM`
6. Importa la base de datos si aún no existe:
   - Usa el archivo `src/database/tareas.sql` en tu gestor MySQL.
7. Ejecuta la aplicación con uv:
   ```powershell
   uv run .
   ```

> Si prefieres no usar `uv`, también puedes ejecutar:
> ```powershell
